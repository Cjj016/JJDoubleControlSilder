...
- (void)setupLine {
    ///进度底轴
    UIView *basicLine = [[UIView alloc] initWithFrame:CGRectMake(0, self.setting.itemSize/2 - self.setting.lineHeight/2, self.frame.size.width, self.setting.lineHeight)];
    ...
    if (self.setting.showStepCount) {
        ...
        NSLog(@"self.frame.size.width/self.setting.stepCount == %f",self.frame.size.width/self.setting.stepCount);
        self.stepCountViews = [NSMutableArray array];
        for (NSUInteger i = 0; i < self.setting.stepCount + 1; i++) {
            UIView *stepView = [[UIView alloc] initWithFrame:CGRectMake(-self.setting.stepSize/2 + self.frame.size.width/self.setting.stepCount * i, self.setting.lineHeight/2 - self.setting.stepSize/2, self.setting.stepSize, self.setting.stepSize)];
...
- (void)setupLeftControl {
    ...
    leftControl.frame = CGRectMake(-self.setting.itemSize/2, 0, self.setting.itemSize, self.setting.itemSize);
...
- (void)setupRightControl {
    ...
    rightControl.frame = CGRectMake(self.frame.size.width - self.setting.itemSize/2, 0, self.setting.itemSize, self.setting.itemSize);
...
- (void)leftEventPan:(UIPanGestureRecognizer*)pan {
    ...
    if (center.x + point.x >= 0 && center.x + point.x <= self.frame.size.width) {
        pan.view.center = CGPointMake(center.x + point.x, self.setting.itemSize/2);
        CGRect copyLineFrame = self.leftLine.frame;
        copyLineFrame.size.width = center.x + point.x;
        self.leftLine.frame = copyLineFrame;
        CGFloat selectLefttValue = (copyLineFrame.size.width * self.setting.maxValue)/self.frame.size.width;
...
- (void)rightEventPan:(UIPanGestureRecognizer*)pan {
    ...
    if (center.x + point.x >= 0 && center.x + point.x <= self.frame.size.width) {
        ...
        copyLineFrame.origin.x = center.x + point.x;
        copyLineFrame.size.width = self.frame.size.width - (center.x + point.x);
        self.rightLine.frame = copyLineFrame;
        CGFloat selectRightValue = copyLineFrame.size.width * self.setting.maxValue/self.frame.size.width;
        NSLog(@"selectRightValue == %f",selectRightValue);
        self.rightValue = selectRightValue;
        if (self.selectValue) {
            self.selectValue(self.leftValue,self.rightValue);
        }
    }
